...
const EditCover = ({ open, setOpen }) => {
    ...
    const handelSubmit = async () => {
        if (file.length > 0) {
            ...
            try {
                const resp = await axios.post(`${process.env.REACT_APP_IMG_BASE_URL}/users/cover_pic`, form, {
                    headers: {
                ...
                if (resp) {
                    setSnackbar("Sucessfully Changed Cover Picture")
                    setTimeout(() => {
                        handleClose()
                    }, 5000)
                }
                ...
                setSnackbar("Error in Changing Cover Picture")
