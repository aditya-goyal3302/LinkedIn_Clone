...
exports.get_connections_for_user = async (user_id) => {
    ...
    return await connections_model.find({ $or:[{sent_to:user_id},{requested_by:user_id}], status: "accepted" }, null, { populate: { path: 'requested_by sent_to', select: '_id username first_name last_name image headline cover' } });
}
...
exports.get_pending_connections = async (user_id) => {
    return await connections_model.find({ sent_to:user_id, status: "pending" }, null, { populate: {  path: 'requested_by', select: '_id first_name last_name image headline cover'} });
}
exports.get_suggestions_for_user = async (user_id) => {
    ...
    const resp = await user_model.find({ _id: { $nin: [...suggestions, user_id] } },null,{limit:12 ,select: '_id username first_name last_name image headline cover' });
    return resp;
}